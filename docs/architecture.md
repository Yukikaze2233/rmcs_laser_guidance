...
当前还包含一条**独立的 WS30 雷达数据接入骨架**，但它尚未接入视觉引导主链：

- WS30 UDP 收包
- WS30 协议包解析（points / imu / status）
- 多包点云组帧
- standalone CLI 调试入口 `tool_lidar_dump`
- 未来 ROS2 bridge 的预留边界
...
## WS30 当前数据流 (standalone scaffold)
...
-> Ws30FrameAssembler
-> Ws30Client
-> tool_lidar_dump (stdout / future raw replay / future PCD export)
```

当前这条链路**只负责把雷达数据稳定收进来并可验证**，还没有：
...
- `Ws30UdpSocket`
  - 负责 WS30 UDP 套接字收发、超时和简单文本命令发送
- `Ws30PacketParser`
  - 负责把 WS30 原始字节流解析为 points / imu / serial_number / startup_status
- `Ws30FrameAssembler`
  - 负责把多包 WS30 点数据拼成完整点云帧
- `Ws30Client`
  - 负责统一轮询 WS30 points frame / imu / status，供 CLI 或未来 bridge 消费
...
- `tool_lidar_dump`
  - 当前 WS30 最小人工入口
  - 用于发起 points / imu / SN 请求、观察收包超时与解析结果
  - 后续会扩展 raw 录制/回放与 PCD 导出

### Future ROS2 Bridge (planned, not implemented yet)

- 保持 `rmcs_laser_guidance_core` ROS-free
- 后续单独增加 ROS2 bridge package
- bridge 只做：参数读取、输入采集、结果发布
- bridge 消费 `Ws30Client` 或更高层 `LidarFrame`，发布 `PointCloud2` / `Imu` / status
- Foxglove / RViz2 视图属于 bridge 层，不属于当前 core
...
- stdout 中的 points / imu / status 摘要
- 点云完整帧计数
- 收包超时与解析错误
...
- `pluginlib`
- `sensor_msgs/msg/PointCloud2` 发布逻辑直接进入 core
- Foxglove / RViz2 UI 逻辑进入 core
- 规划器
- 控制指令
- `fire_control`
