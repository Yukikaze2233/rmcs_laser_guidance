...
auto resolve_model_path(
    const rmcs_laser_guidance::InferenceConfig& config, int argc, char** argv)
    -> std::filesystem::path {
...
auto print_value_infos(
    std::string_view label, const std::vector<rmcs_laser_guidance::ModelValueInfo>& values) -> void {
    std::println("{} count={}", label, values.size());
    for (const auto& value : values) {
        std::println(
            "  name={} dtype={} shape={}", value.name, value.element_type, format_shape(value.shape));
    }
}
